...
from plan_and_track import Controller
import C3M
...
class Cityscape(object):
    ...
    def __init__(self, num_agents, area_size=20, show=False):
        self.num_agents = num_agents
        self.area_size = area_size
        self.area_bound = None
        self.obstacles = None
        self.show = show
    ...
    def reset(self):
        ...
        self.reset_reference_controllers()
    ...
    def reset_reference_controllers(self):
        # obs_p1, obs_p2 = np.split(np.array(self.OBSTACLES), 2, axis=1)
        # obs_p1 = np.concatenate([obs_p1, np.zeros_like(obs_p1[:, :1])], axis=1)
        # obs_p2 = np.concatenate(
        #     [obs_p2, self.area_size * np.ones_like(obs_p2[:, :1])], axis=1)
        # obstacles = np.concatenate([obs_p1, obs_p2], axis=1)
        obstacles = self.obstacles
        x_bound = self.area_bound
        bloat_factor = self.bloat_factor

        self.C3M = C3M.get_model('data/model_best.pth.tar')
        controllers = []
        for i in range(self.num_agents):
            x_init = np.concatenate([self.waypoints[i][0], np.zeros(5)])
            # x_bound = np.array([(0, 1), (0, 1), (0, 1)]) * self.area_size
            controller = Controller(
                self.C3M, x_init, self.waypoints[i][1:], x_bound, obstacles, bloat_factor, self.num_segs, self.MIPGap)
            controllers.append(controller)
        self.controllers = controllers

    def get_reference_control(self, s):
        assert s.shape[0] == self.num_agents
        u_ref, x_ref = [], []
        for i in range(self.num_agents):
            ur, xr = self.controllers[i](s[i])
            u_ref.append(ur)
            x_ref.append(xr)
        u_ref = np.array(u_ref).reshape(-1, 3)[:, [1, 2, 0]]
        x_ref = np.array(x_ref).reshape(-1, 8)
        return u_ref, x_ref


class Maze(Cityscape):

    def __init__(self, num_agents=64, show=False):

        scale = np.sqrt(max(1.0, num_agents / 64.0))
        area_size = 20 * scale

        Cityscape.__init__(self, num_agents, area_size, show)
        #self.OBSTACLES = np.array([
        #    [3, 3, 6, 4], [3, 4, 4, 16], [3, 16, 6, 17],
        #    [8, 7, 12, 8], [8, 12, 12, 13],
        #    [11, 3, 12, 8], [11, 12, 12, 17],
        #    [10, 3, 17, 4], [10, 16, 17, 17],
...
def network_action(s, u_ref, obs_radius=1.0, indices=None):
    """ Controller as a neural network.
    Args:
        s (N, 8): The current state of N agents.
        u_ref (N, 3): The reference control input.
...
def loss_actions(s, u, u_ref, indices):
