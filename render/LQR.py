...
x_lb = np.array([-np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.pi/3, -np.pi/3])
x_ub = np.array([ np.inf,  np.inf,  np.inf,  np.inf,  np.inf,  np.inf,  np.pi/3,  np.pi/3])

# x_lb = np.array([-np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf])
# x_ub = np.array([ np.inf,  np.inf,  np.inf,  np.inf,  np.inf,  np.inf,  np.inf,  np.inf])

def odeint(f, x0, t, args=()):
    x = [np.array(x0),]
    for idx in range(len(t)-1):
        dot_x = f(x[-1], t[idx], *args)
        xnext = x[-1] + dot_x*(t[idx+1]-t[idx])
        xnext[xnext<x_lb] = x_lb[xnext<x_lb]
        xnext[xnext>x_ub] = x_ub[xnext>x_ub]
        x.append(xnext)
    return np.array(x)
